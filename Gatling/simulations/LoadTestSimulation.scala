...
class LoadTestSimulation extends Simulation {
  ...
  // ------------------------------------------
  // Paths igual que K6 / JMeter / Pulse
  // ------------------------------------------
  val paths = List(
    "/onlineshop/",
  ...
  // ------------------------------------------
  // Scenario igual a K6/JMeter/Pulse
  // ------------------------------------------
  val scn = scenario("Load Test Simulation")
    .repeat(10) { // 10 iterations por usuario
      foreach(paths, "path") {
        exec(
          http("GET ${path}")
            .get("${path}")
  ...
  // ------------------------------------------
  // Injection model equivalente
  // ------------------------------------------
  setUp(
    scn.inject(
      nothingFor(10),   // Delay inicial → igual que K6
      atOnceUsers(2)    // 2 usuarios virtuales → igual que JMeter/K6/Pulse
    )
